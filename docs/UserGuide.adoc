...
* `SEMESTER` is in the format `y/years/sem[/s]`, with `year` indicating the year, `sem` indicating the semester and `[/s]` indicating whether the semester is a special semester.`year` must be a positive integer lower than 7, and `sem` must be either 1 or 2.
